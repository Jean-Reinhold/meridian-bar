...
|---|---------|----------------------|-----------|
| F1 | **All-accounts menu bar label** — one colored segment per profile: `alias worst%` (e.g. `paul 91 · pnr 52 · rein 32`) | Label shows every configured profile with its worst-window %, colored by status, readable in light + dark menu bars | M1 |
...
| F10 | **Settings** — base URL (default `http://127.0.0.1:3456`), poll interval, label style, per-profile alias overrides | Settings persist in `UserDefaults`; base URL change takes effect without relaunch | M3 |
...
| F13 | **Release pipeline & security CI** — CodeQL + dependency review + OpenSSF Scorecard; CI-only release builds: universal binary, signed + notarized, checksummed zip on tag | Security workflows green on `main`; `git tag vX.Y.Z` produces a notarized, runnable artifact with published sha256 (`okf/06`) | M4 |
| F14 | **Easy install** — Homebrew tap cask auto-bumped by the release workflow; README install section (cask, zip, source) | `brew install --cask jean-reinhold/tap/meridian-bar` installs the current release | M4 |
